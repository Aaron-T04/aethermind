...
export const testConnection = action({
  ...
  handler: async ({ runMutation, runQuery }, { id }) => {
    // Import internal API - use type assertion to bypass type checking during initial generation
    // Once types are generated, this will be properly typed
    const apiModule = await import("./_generated/api");
    const internalApi = apiModule.internal as any;
    
    const server = await runQuery(internalApi.mcpServers.getMCPServer, { id });
    ...
    try {
      ...
      await runMutation(internalApi.mcpServers.updateMCPServer, {
        id,
